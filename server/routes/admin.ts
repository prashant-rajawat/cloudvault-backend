  ...
  try {
    ...
    const usersList = Array.from(allUserIds).map((id) => {
      ...
      return {
        ...
        storageQuotaBytes: (!prof?.storage_quota_bytes || prof.storage_quota_bytes === 5368709120) ? DEFAULT_STORAGE_QUOTA_BYTES : prof.storage_quota_bytes, // 15 GB default
  ...
  try {
    ...
    res.json({
      ...
      user: {
        ...
        storageQuotaBytes: (!profile?.storage_quota_bytes || profile.storage_quota_bytes === 5368709120) ? DEFAULT_STORAGE_QUOTA_BYTES : profile.storage_quota_bytes,
        storageUsedBytes,
    ...
    const topUsers = Object.entries(userStorageMap)
      .map(([userId, usedBytes]) => {
        ...
        return {
          ...
          usedBytes,
          quotaBytes: (!prof?.storage_quota_bytes || prof.storage_quota_bytes === 5368709120) ? DEFAULT_STORAGE_QUOTA_BYTES : prof.storage_quota_bytes,
        };
      })
